...
sub run_single_task {
	...
	my $archs_dir;
	my $sdc_dir = "sdc";
	my $script_default = "open_fpga_run_vtr_flow.pl";
	...
	if ( $script eq $script_default ) {
		# This is hack to automatically add the option '-temp_dir .' if using the open_fpga_run_vtr_flow.pl script
	...
	system( "rm -rf current-run" );
    system( "ln -s $run_prefix$experiment_number current-run" );
		...
		if ( $processors == 1 ) {
			foreach my $circuit (@circuits) {
				foreach my $arch (@archs) {
                    ...
                    my @arch_split = split('_', $arch);
                    my $track_num = $arch_split[5];

					# SDC file defaults to circuit_name.sdc					
					my $sdc = fileparse( $circuit, '\.[^.]+$' ) . ".sdc";
					system(
						"$script_path $circuits_dir/$circuit $archs_dir/$arch -sdc_file $sdc_dir/$sdc $script_params -track_w $track_num\n"
					);
				}
			}
		}
