...
#include "vpr_types.h"
#include "globals.h"
#include "bitgen_output_clustering.h"
...
static void bitgen_print_tabs(FILE *fpout, int num_tabs) {
	int i;
...
static void bitgen_print_string(const char *str_ptr, int *column, int num_tabs, FILE * fpout) {
	...
	if (*column + len + 2 > LINELENGTH) {
		fprintf(fpout, "\n");
		bitgen_print_tabs(fpout, num_tabs);
...
static void bitgen_print_net_name(int inet, int *column, int num_tabs, FILE * fpout) {
	...
	bitgen_print_string(str_ptr, column, num_tabs, fpout);
}

static void bitgen_print_interconnect(int inode, int *column, int num_tabs,
		FILE * fpout) {
	...
	if (rr_node[inode].net_num == OPEN) {
		bitgen_print_string("open", column, num_tabs, fpout);
	} else {
		str_ptr = NULL;
		...
		if (prev_node == OPEN
			...
			bitgen_print_net_name(rr_node[inode].net_num, column, num_tabs, fpout);
		} else {
			name =
...
static void bitgen_print_open_pb_graph_node(t_pb_graph_node * pb_graph_node,
	...
	bitgen_print_tabs(fpout, tab_depth);
		...
		for (i = 0; i < pb_type->num_ports; i++) {
			if (!pb_type->ports[i].is_clock
				...
				for (j = 0; j < pb_type->ports[i].num_pins; j++) {
					...
					bitgen_print_interconnect(node_index, &column, tab_depth + 2,
							fpout);
				}
				fprintf(fpout, "</port>\n");
				port_index++;
		...
		if (pb_type->num_modes > 0) {
			for (i = 0; i < mode->num_pb_type_children; i++) {
				...
				for (j = 0; j < mode->pb_type_children[i].num_pb; j++) {
					...
					bitgen_print_open_pb_graph_node(
...
static void bitgen_print_pb(FILE *fpout, t_pb * pb, int pb_index, int tab_depth) {
	...
	if (pb_type->num_modes == 0) {
	    // fprintf(fpout, "Zero Mode\n");
        if (strncmp(pb_type->name, "ff", 2) == 0)
        {
            fprintf(fpout, "        FF: %s\n", pb->name);
        }
        else if (strncmp(pb_type->name, "lut", 3) == 0)
        {
            // Do Nothing
        }
        else if (strncmp(pb_type->name, "io", 2) == 0)
        {
            // Do Nothing
        }
        else
    ...
    {
        if (strncmp(mode->name, "lut", 3) == 0)
        {
            // Do Nothing
        }
        else if (strncmp(pb_type->name, "soft_logic", 10) == 0)
        {
            // Do Nothing
        }
        else if (strncmp(mode->name, "outpad", 6) == 0)
        {
            // Do Nothing
        }
        else if (strncmp(mode->name, "inpad", 5) == 0)
        {
            // Do Nothing
        }
        else if (strncmp(mode->name, "ble", 3) == 0)
        {
            // BLE TYPE
            fprintf(fpout, "    BLE %s\n", pb->name);
            // INPUTS
                ...
                if (!pb_type->ports[i].is_clock && pb_type->ports[i].type == IN_PORT) {
                    for (j = 0; j < pb_type->ports[i].num_pins; j++) {
                        ...
                        if (pb_type->parent_mode == NULL) {
                            bitgen_print_net_name(rr_node[node_index].net_num, &column, tab_depth, fpout);
                        } else {
                            bitgen_print_interconnect(node_index, &column, tab_depth, fpout);
                        }
                    }
                    port_index++;
                }
                ...
                for (i = 0; i < mode->num_pb_type_children; i++) {
                    for (j = 0; j < mode->pb_type_children[i].num_pb; j++) {
                        ...
                        if ((pb->child_pbs[i] != NULL) && (pb->child_pbs[i][j].name != NULL)) {
                            bitgen_print_pb(fpout, &pb->child_pbs[i][j], j, tab_depth + 1);
                            FF_PRINTED = 1;
                        }
                        else
                        {
            ...
            if (FF_OPEN == 1)
            {
                fprintf(fpout, "        FF: OPEN\n");
            }
        }   
        else if (strncmp(mode->name, "clb", 3) == 0)
        {
            fprintf(fpout, "CLB %s\n", pb->name);
            ...
            if (pb_type->num_modes > 0) {
                for (i = 0; i < mode->num_pb_type_children; i++) {
                    for (j = 0; j < mode->pb_type_children[i].num_pb; j++) {
                        /* If child pb is not used but routing is used, I must print things differently */
                        if ((pb->child_pbs[i] != NULL) && (pb->child_pbs[i][j].name != NULL)) {
                            bitgen_print_pb(fpout, &pb->child_pbs[i][j], j, tab_depth);
                        }
                        else
                        {
                            fprintf(fpout, "    BLE OPEN\n");
                        }
                    }
                }
            }
...
static void bitgen_print_clusters(t_block *clb, int num_clusters, FILE * fpout) {
	...
	for (icluster = 0; icluster < num_clusters; icluster++) {
		...
		bitgen_print_pb(fpout, clb[icluster].pb, icluster, 0);
	}
}
...
void bitgen_output_clustering(t_block *clb, int num_clusters, boolean global_clocks,
	...
	//		bitgen_print_string(logical_block[bnum].name, &column, 2, fpout);
	//	}
	// }
	// fprintf(fpout, "\n\t</inputs>\n");
	// fprintf(fpout, "\n\t<outputs>\n\t\t");
	...
	//		if (is_clock[netnum]) {
	//			bitgen_print_string(vpack_net[netnum].name, &column, 2, fpout);
	//		}
	//	}
	//	fprintf(fpout, "\n\t</clocks>\n\n");
	// }
	...
	if (skip_clustering == FALSE)
		bitgen_print_clusters(clb, num_clusters, fpout);

	// fprintf(fpout, "</block>\n\n");

	fclose(fpout);

	// bitgen_print_stats(clb, num_clusters);
}
