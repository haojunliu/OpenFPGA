...
#ifdef OPENFPGA
    char * bitgen_out_fname;
    char * bitgen_pack_surfix = ".reRoute";
    bitgen_out_fname = (char *)malloc(strlen(out_fname) + strlen(bitgen_pack_surfix) + 2);
    sprintf(bitgen_out_fname, "%s%s", out_fname, bitgen_pack_surfix);

    bitgen_output_clustering(clb, num_clb, global_clocks, is_clock, bitgen_out_fname, FALSE);
#endif
