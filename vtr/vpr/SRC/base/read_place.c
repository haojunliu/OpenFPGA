...
void print_place(char *place_file, char *net_file, char *arch_file) {
    ...
    char * bit_gen_place_file;
    char * bitGen_surfix = ".bitGen";
    bit_gen_place_file = (char *)malloc(strlen(place_file) + strlen(bitGen_surfix) + 2);
    sprintf(bit_gen_place_file, "%s%s", place_file, bitGen_surfix);

    bitGen_fp = fopen(bit_gen_place_file, "w");
    
    fprintf(bitGen_fp, "Netlist file: %s   Architecture file: %s\n", net_file,
			arch_file);
	fprintf(bitGen_fp, "Array size: %d x %d logic blocks\n\t\n", nx, ny);
	fprintf(bitGen_fp, "#block type\t\tblock name\t\t\tx\t\ty\tsubblk\tblock number\n");
	fprintf(bitGen_fp, "#----------\t\t----------\t\t\t--\t\t--\t------\t------------\n");

	for (i = 0; i < num_blocks; i++) {
        fprintf(bitGen_fp, "%s       ", "top_level");
		fprintf(bitGen_fp, "%s\t\t", block[i].name);
		if (strlen(block[i].name) < 8)
			fprintf(bitGen_fp, "\t");

		fprintf(bitGen_fp, "%d\t\t%d\t\t%d", block[i].x, block[i].y, block[i].z);
		fprintf(bitGen_fp, "\t\t#%d\n", i);

        t_pb * pb = block[i].pb;

        if(pb != NULL && pb->pb_graph_node != NULL)
        {
            int j, k;
            for (j = 0; j < pb->pb_graph_node->pb_type->modes[pb->mode].num_pb_type_children; j++)
            {
                if(pb->child_pbs[j] != NULL)
                {
                    for(k = 0; k < pb->pb_graph_node->pb_type->modes[pb->mode].pb_type_children[j].num_pb; k++)
                    {
                        fprintf(bitGen_fp, "sub_level       %s", pb->child_pbs[j][k].name);
                        
                        if (pb->child_pbs[j][k].name!=NULL && strlen(pb->child_pbs[j][k].name) < 8)
                            fprintf(bitGen_fp, "\t");

                        fprintf(bitGen_fp, "\t\t%d\t\t%d", j, k);

                        if ( pb->child_pbs[j][k].lut_pin_remap != NULL)
                        {
                            fprintf(bitGen_fp, "\tpin_swapped\n");
                        }
                        else
                        {
                            fprintf(bitGen_fp, "\t\n");
    ...
    char * reRoute_place_file;
    char * reRoute_surfix = ".reRoute";
    reRoute_place_file = (char *)malloc(strlen(place_file) + strlen(reRoute_surfix) + 2);
    sprintf(reRoute_place_file, "%s%s", place_file, reRoute_surfix);

    reRoute_fp = fopen(reRoute_place_file, "w");
    
	for (i = 0; i < num_blocks; i++) {

		fprintf(reRoute_fp, "%s\t\t", block[i].name);
		if (strlen(block[i].name) < 8)
			fprintf(reRoute_fp, "\t");

		fprintf(reRoute_fp, "%d\t\t%d\n", block[i].x, block[i].y);
    }
