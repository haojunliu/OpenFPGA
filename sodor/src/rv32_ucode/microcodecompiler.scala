...
object MicrocodeCompiler
{
   ...
   def generateInstructionList (): Map[String,Bits] =
   {
      var inst_list = Map[String, Bits]();
      ...
      for (m <- instClass.getMethods()) 
      {
         ...
         if (rtype == bitsClass) 
         {
            ...
            inst_list += ((name, i.asInstanceOf[Bits])); 
         }
      }
   ...
   def generateDispatchTable (labelTargets: Map[String,Int]): Array[(Bits, UFix)]=
   {
      println("Generating Opcode Dispatch Table...");
      var dispatch_targets = ArrayBuffer[(Bits,UFix)]();
